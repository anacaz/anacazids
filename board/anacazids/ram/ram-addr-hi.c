...
FORWARD int ram_addr_hi_test(void *memory, int size);
FORWARD int ram_addr_hi_main(int argc, char **argv);

/*
 * TEST-2: addr_stuck_hi_test
 *
 * 1) Write base location w/background pattern
 * 2) Write one address location w/foreground pattern
 * 3) Read base location to confirm background pattern
 * 4) FAIL if forground pattern found
 * 5) FAIL if background pattern not found
 * 6) PASS if background pattern found
 */
int ram_addr_hi_test(void *memory, int size)
{
	...
	/*
	 * Write the base location with background pattern.
	 */
	*(unsigned long *)start = LONG(background);
	...
	printf("\t[ %s get BG location:%08X = %08X ]\n", program,
		0, *(unsigned long *)0);
	if (start && *(unsigned long *)0 == LONG(background))
		return(-1);
	...
	for (mask = 4L; mask && mask <= stop; mask <<= 1L)
	{
		...
		*(unsigned long *)location = LONG(foreground);
		printf("\t[ %s set FG location:%08X = %08X ]\n", program,
			location, *(unsigned long *)location);

		content = *(unsigned long *)start;
		printf("\t[ %s get BG location:%08X = %08X ]\n", program,
			start, content);
...
int ram_addr_hi_main(int argc, char **argv)
{
	void *memory;
	int len;
	...
	if (ram_addr_hi_test(memory, len) == -1)
		return(-1);
	return(0);
}
